...
BANNER = ("<!-- Generated by scripts/update-history.py from\n"
          "     %s\n"
...
def convert_html(page, source_url, platform=None):
...
def convert_voyager(text, source_url):
    ...
    out = ["# Voyager Firmware History", "", BANNER % source_url] + body
    text = "\n".join(out)
    text = re.sub(r"\n{3,}", "\n\n", text)
    return text.rstrip("\n") + "\n"
...
def main():
    ...
    for path, url, platform, convert in jobs:
        ...
        if convert is convert_html:
            text = convert(source, url, platform)
        else:
            text = convert(source, url)
